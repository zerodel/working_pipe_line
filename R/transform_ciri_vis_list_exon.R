...
adhoc_deco_lst_df_with_anno_info <- function(df_raw_region, df_list) {
...
summarize_region_empty <- function(df_circexon) {
  ...
  lst_region <-
    lapply(split(df_circexon, df_circexon$bsj), confirm_empty_region)
...
if (dim(df_partial_isoform)[1] < 1) {
  ...
  df_raw_region <- adhoc_deco_lst_df_with_anno_info(df_raw_region, df_partial_isoform)
  
  df_region_circexon <- base::subset(df_raw_region, !(df_raw_region$start == 0 | df_raw_region$end == 0))
  
  df_region_empty <- summarize_region_empty(df_raw_region)
  df_region_empty <- adhoc_deco_lst_df_with_anno_info(df_region_empty, df_partial_isoform)
  df_region_empty$start_bsj <- df_region_circexon$start_bsj[base::match(df_region_empty$bsj, df_region_circexon$bsj)]
  df_region_empty$end_bsj <- df_region_circexon$end_bsj[base::match(df_region_empty$bsj, df_region_circexon$bsj)]
