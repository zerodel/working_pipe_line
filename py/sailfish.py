...
INDEX_SECTION = "SAILFISH_INDEX"
QUANTIFY_SECTION = "SAILFISH_QUANTIFY"
...
def _check_index_options(para_dict):
    with py.body.cli_opts.OptionChecker(para_dict) as opt_checker:
        opt_checker.must_have("--transcripts", os.path.exists, FileNotFoundError("Error@SAILFISH_INDEX: unable to find the transcript fa"),
        ...
        opt_checker.may_need(_OPT_KMER_SIZE, lambda x: x.isdecimal() and 0 < int(x) < 32,
                             ValueError("ERROR@SAILFISH_INDEX: given incorrect Kmer length"),
        ...
        opt_checker.may_need(_THREADS, lambda x: x.isdecimal() and  0 < int(x) <= py.body.utilities.core_numbers_of_cpu(),
                             ValueError("ERROR@SAILFISH_INDEX: given incorrect threads number as {}".format(opt_checker.dict_opt["--threads"])),
...
def _check_quantify_options(para_dict):
    with py.body.cli_opts.OptionChecker(para_dict) as check_args:
        ...
        if _OPT_MATE1 in para_dict and _OPT_MATE2 in para_dict:
            if _OPT_UN_MATED_READS in para_dict:
                raise KeyError("ERROR@SAILFISH_QUANTIFY_reads_mode: can not using paired-end and single-end at the same time")
