...
INDEX_SECTION = "SALMON_INDEX"
QUANTIFY_SECTION = "SALMON_QUANTIFY"
...
def get_index_path(para_config=None, *args, **kwargs):
    updated_para = py.body.cli_opts.merge_parameters(kwargs, para_config, "SALMON_INDEX")
...
def interpret_seq_files(input_files):
    if input_files:
        if isinstance(input_files, list):
            ...
            raise TypeError("Error@SALMON_interpret_seq_file: only list and string are allowed in assign SALMON input_files")
    else:
...
def index(para_config=None, *args, **kwargs):
    opts_raw = py.body.cli_opts.merge_parameters(kwargs, para_config, INDEX_SECTION)
...
def quantify(para_config=None, *args, **kwargs):
    opts_raw = py.body.cli_opts.merge_parameters(kwargs, para_config, QUANTIFY_SECTION)
    opts = copy.copy(opts_raw)

    _check_quantify_options(opts)
...
def _get_cmd_make_index(para_dict):
    ...
    str_salmon_index_command += " " + py.body.cli_opts.enum_all_opts(para_dict)
    return str_salmon_index_command


def _check_index_options(para_dict):
    with py.body.cli_opts.OptionChecker(para_dict) as opt_checker:
        ...
        opt_checker.one_and_only_one(["-i", _INDEX], py.body.cli_opts.is_suitable_path_with_prefix,
        ...
        opt_checker.at_most_one(["-s", "--sasamp"], _is_legal_suffix_array_interval,
                                ValueError("ERROR@SALMON_INDEX: the interval of suffix array sampled should be a power of 2"),
        ...
        opt_checker.may_need(_PERFECT_HASH, lambda x: not x,
                             ValueError("ERROR@SALMON_INDEX: perfect-hash is a just a flag need no value , and should be used only in quasi mode"),
...
def _check_quantify_options_alignment_mode(para_dict):
    with py.body.cli_opts.OptionChecker(para_dict) as opt_checker:
        ...
        _check_options_shared_by_both_salmon_quantify_modes(opt_checker, para_dict)

        opt_checker.forbid_these_args("-i", _INDEX)


def _check_options_shared_by_both_salmon_quantify_modes(opt_checker, para_dict):
...
def _check_quantify_options_reads_mode(para_dict):
    with py.body.cli_opts.OptionChecker(para_dict) as opt_checker:
        _check_options_shared_by_both_salmon_quantify_modes(opt_checker, para_dict)
        ...
        if _OPT_MATE1 in para_dict and _OPT_MATE2 in para_dict:
            if _OPT_UN_MATED_READS in para_dict:
                raise KeyError("ERROR@SALMON_QUANTIFY_reads_mode: can not using paired-end and single-end at the same time")
