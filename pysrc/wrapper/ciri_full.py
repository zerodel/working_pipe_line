...
def _check_opts(opts=None):
    ...
    check_your_option.must_have(_OPT_JAR_FULL_PATH, os.path.exists,
                                FileNotFoundError("Error: can not find CIRI-full jar file"),
                                "path to CIRI-full.jar")

    check_your_option.must_have("-1", os.path.exists,
                                FileNotFoundError("Error: unable to find pair-end reads file 1 "),
                                "path to PE reads 1")

    check_your_option.must_have("-2", os.path.exists,
                                FileNotFoundError("Error: unable to find pair-end reads file 2 "),
                                "path to PE reads 2")

    check_your_option.must_have("-a", os.path.exists,
                                FileNotFoundError("Error@CIRI-AS: incorrect annotation file provide for CIRI-full"),
    ...
    check_your_option.must_have("-l", lambda x: isinstance(int(x), int),
                                TypeError("Error@CIRI-full , improper read length given"),
                                "read length")
...
def detect(para_config=None, **kwargs):
    ...
    r1, r2, ref, anno, = opts_raw.get("-1"), opts_raw.get("-2"), opts_raw.get("-r"), opts_raw.get("-a")
    ...
    sub_dir_detection, sub_dir_vis = detection_sub_dir_under(dir_out), vis_sub_dir_under(dir_out)

    ut.do_make_dir(dir_out)
    ut.do_make_dir(sub_dir_detection)
    _logger.debug("ciri-full detection report will be put in {}".format(sub_dir_detection))
    _logger.debug("ciri-full visualization report will be put in {}".format(sub_dir_vis))
    ...
    ciri_args_dict = dict(pysrc.body.config.load_or_update_option_section(pysrc.wrapper.ciri.SECTION_DETECT))
    ...
    _logger.debug("and ciri running under the following  parameters: {}".format(str(ciri_args_dict)))
    ...
    if not pysrc.file_format.ciri_entry.is_ciri_file_intact(ciri_file):
        _logger.warning("WARNING: CIRI may meet some ERROR , report file : {}".format(ciri_file))
    else:
        _logger.debug("seems ciri works well ")

    # run ciri-as
    ciri_as_args_dict = dict(pysrc.body.config.load_or_update_option_section(pysrc.wrapper.ciri_as.SECTION_DETECT))
    ...
    _logger.debug("CIRI-AS running under the parameters: {}".format(str(ciri_as_args_dict)))
    ...
    # phase Merge
    cmd_merge = _get_cmd_merge(jar_full, ref, anno, ciri_file, ciri_as_prefix, ciri_full_prefix)
    _logger.debug("command merge of ciri-full is {cmd_merge}".format(cmd_merge=cmd_merge))
    pysrc.body.worker.run(cmd_merge)

    # phase Vis
    cmd_vis = _get_cmd_vis(jar_vis, ref, ciri_full_prefix, sub_dir_vis)
    _logger.debug("command vis of ciri-full is {cmd_vis}".format(cmd_vis=cmd_vis))
    pysrc.body.worker.run(cmd_vis)

    fa_reconstructed = glob.glob(os.path.join(sub_dir_vis, "*.fa"))
    _logger.debug("after phase vis , we got fa files as : {}".format(str(fa_reconstructed)))
    _logger.debug("try to combine reconstructed fa file into one file: {}....".format(fa_target))
...
def rebuild_fa_path_under(dir_out):
    ...
    fa_path = [x for x in os.listdir(your_sub_dir) if x.endswith(".fa") or x.endswith(".fasta")]
...
def _get_cmd_vis(jar_vis, ref, ciri_full_prefix, output_dir):
    detail_anno = "".join([ciri_full_prefix, "_merge_circRNA_detail.anno"])
    cmd = "java -jar {jar_vis} -i {merged} -d {out_dir} -r {ref}".format(jar_vis=jar_vis,
                                                                         merged=detail_anno,
                                                                         out_dir=output_dir,
                                                                         ref=ref)
    return cmd
...
def interpret_seq_files():
    _logger.debug("this function : interpret_seq_files@ciri_full is not implemented yet")
    pass
...
def translate_vis_list(path_vis_list, tmp_dir=""):
    ...
    path_bed_circexon, path_bed_blank = os.path.join(dir_parpar, "circexon.bed"), os.path.join(dir_parpar, "blank.bed")
    ...
    _logger.debug("circexon bed file will be : {circexon},\\"
                  " blank bed will be : {blank}".format(circexon=
                                                        path_bed_circexon,
                                                        blank=path_bed_blank))

    cmd_filter_r = " ".join(["Rscript", path_r_script, path_vis_list, path_bed_circexon, path_bed_blank])

    _logger.debug("""external cmd for filter ciri-full vis list : \n {}""".format(cmd_filter_r))
...
def filter_out_un_touched_circular_rna(path_bed_ciri, path_vis_list, tmp_dir=""):
    _logger.debug("start filter out those BSJ which contains partial inner structure information")

    path_r_script = _find_that_r_script(BASE_NAME_R_SCRIPT_FILTER_BSJ)

    _logger.debug("external R script will be invoked : {}".format(path_r_script))
    ...
    cmd_r = " ".join(["Rscript", path_r_script, path_bed_ciri, path_vis_list, path_bed_out])
...
def _find_that_r_script(basename_r):
    ...
    dir_r = os.path.dirname(os.path.dirname(os.path.dirname(path_current_script)))
...
def exons_within_blank_region(bed_file_show_blank_region, genomic_annotation):
    ...
    for single_exon in exons_inside_blank_region:
        single_exon.set_gene_id(host_gene_of.get(single_exon.get_transcript_id()))
...
def _rename_rebuild_fa(fa_file_in, use_suffix=True):
    ...
    for seq in fa_in:
        ...
        if short_id_isoform not in dict_fa_id:
            ...
            dict_fa_id[short_id_isoform] += 1
            short_id_isoform = ".".join([short_id_isoform, str(dict_fa_id[short_id_isoform])])
